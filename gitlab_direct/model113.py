...
class Namespaces(BaseModel):
    ...
    trial_ends_on = DateTimeField(null=True)
...
class Projects(BaseModel):
    ...
    class Meta:
        db_table = 'projects'
        indexes = (
            (('id', 'last_repository_updated_at', 'repository_storage'), False),
...
class ApplicationSettings(BaseModel):
    ...
    password_authentication_enabled_for_git = BooleanField(null=True)
    ...
    snowplow_collector_uri = CharField(null=True)
    snowplow_cookie_domain = CharField(null=True)
    snowplow_enabled = BooleanField()
    snowplow_site = CharField(db_column='snowplow_site_id', null=True)
...
# Create a reference object to stand in for our as-yet-undefined MergeRequestDiffs model.
...
class MergeRequests(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            (('iid', 'target_project'), False),
            (('iid', 'target_project'), True),
...
class Epics(BaseModel):
    assignee = ForeignKeyField(db_column='assignee_id', null=True, rel_model=Users, to_field='id')
    author = ForeignKeyField(db_column='author_id', rel_model=Users, related_name='users_author_set2', to_field='id')
    cached_markdown_version = IntegerField(null=True)
    created_at = DateTimeField()
    ...
    due_date_sourcing_milestone = IntegerField(db_column='due_date_sourcing_milestone_id', null=True)
    ...
    start_date_sourcing_milestone = IntegerField(db_column='start_date_sourcing_milestone_id', null=True)
    title = CharField()
...
class Issues(BaseModel):
    ...
    class Meta:
        db_table = 'issues'
        indexes = (
            (('id', 'created_at', 'state', 'project'), False),
            (('project', 'id', 'state', 'due_date'), False),
            (('project', 'iid'), True),
            (('project', 'state', 'id', 'updated_at'), False),
        )
...
class PrometheusAlerts(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            (('project', 'prometheus_metric'), True),
        )
...
class SiteStatistics(BaseModel):
    repositories_count = IntegerField()
    wikis_count = IntegerField()
...
class Taggings(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            ...
            (('tag', 'context', 'tagger_type', 'tagger', 'taggable_type', 'taggable'), True),
...
class VulnerabilityFeedback(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            (('category', 'project', 'project_fingerprint', 'feedback_type'), True),
        )
